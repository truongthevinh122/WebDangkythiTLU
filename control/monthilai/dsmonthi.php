...
<div class="panel-heading">
  ...
  <?php include('../control/monthilai/importForm.php') ?>
  <?php include('../control/monthilai/AddForm.php') ?>
...
<table class="table table-striped table-bordered table-hover" id="dataTables-monthi">
    ...
    <tbody>
      ...
      for ($i=0; $i < count($monthi['data']); $i++) {
        if ($monthi['data'][$i]['kyhoc_id']==$filterky and $monthi['data'][$i]['monthi_mota']==1) {
          ?>
          <tr class="odd gradeX">
            <td></td>
            ...
            <td hidden><?php echo $monthi['data'][$i]['monthi_id'] ?></td>
              <td class="center">
                <a href="/api.dangkythi/control/monthilai/delete.php?kyhoc_id=<?php echo $filterky ?>&sumbitKy=&monthi_id=<?php echo $monthi['data'][$i]['monthi_id'] ?>"
